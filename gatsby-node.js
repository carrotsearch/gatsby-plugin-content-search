...
const generateTypeName = type => _.camelCase(`${TYPE_PREFIX} ${type}`);
...
const createPages = async (
  ...
  { name = DEFAULT_NAME, query = DEFAULT_QUERY,
    searchableFields = DEFAULT_SEARCHABLE_FIELDS, normalizer = DEFAULT_NORMALIZER }) => {
  ...
  reporter.info(`Search index uncompressed size: ${(size / 1024.0).toFixed(1)} kB.`);
...
const createResolvers = async (
  ...
  createResolvers({
    Query: {
      [_.lowerFirst(generateTypeName(name))]: {
